...
msg2decode = '0f351c71c7654ff251de056ea68920cf2d7d49e53ff9174bd303fc04d74e7e69ad9cb9bf56160c2aea960d002b139b6b8f25982fe2e6e9f158a2451944c3f623d19dc425648beceb621f38768abb4f47ad46176b7b04b3c069a0c4da3b0dd3bea96a54b7e4453989f86b55ba8b635b90f944'
...
#CRACK='stylE used To be an interaction between the human Soul and tools that were limiting In the digital Era it i l have to come from the soul alone'
#CRACK='we aRe the Children of a technological age We havefound streamlined ways of doing much of our routIne work  r nting is so longer the only way is reproduc'
...
#-------  code  -----
    for i in range(0, 12): ###
		result = encrypt(interception[i].decode('hex'), msg2decode.decode('hex')) ###

		if len(msg2decode)<=len(interception[i]): ###
			for j in range(0, len(interception[12].decode('hex'))): ###
			...
			print ("") ###
		if len(msg2decode)>len(interception[i]): ###
			for j in range(0, len(interception[i].decode('hex'))): ###
				if (cmp(result[j], 'a') >= 0 and cmp('z', result[j]) >= 0) or (cmp(result[j], 'A') >= 0 and cmp('Z', result[j]) >= 0): ###
					print (result[j]), ###
				else: ###
					print ('_'), ###
